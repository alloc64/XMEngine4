...
void Skybox::RenderSun(void) {
    glPushMatrix();
    {
        glEnable(GL_BLEND);
        {
            ...
            vec3 pos = vec3(level.mainLight.center.x,
                            level.mainLight.center.y - 800,
                            level.mainLight.center.z);

            int width = 500, height = 500;
            float mat[16];
            glGetFloatv(GL_MODELVIEW_MATRIX, mat);

            vec3 right = Normalize(vec3(mat[0], mat[4], mat[8])) * width * (camera.GetCameraDirection().y * 2);
            vec3 up = Normalize(vec3(mat[1], mat[5], mat[9])) * height * (camera.GetCameraDirection().y * 2);
