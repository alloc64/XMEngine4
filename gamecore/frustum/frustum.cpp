...
bool Frustum::PointInFrustum(vec3 vec) {
    for (int i = 0; i < 6; i++) {
        if (FrustumMatrix[i][0] * vec.x + FrustumMatrix[i][1] * vec.y + FrustumMatrix[i][2] * vec.z +
            FrustumMatrix[i][3] <= 0)
            return false;
    }
    return true;
...
bool Frustum::PolygonInFrustum(vec3 v0, vec3 v1, vec3 v2) {
    if (PointInFrustum(v0) && PointInFrustum(v1) && PointInFrustum(v2)) return false;
    return true;
}
...
bool Frustum::IsPointOccluded(vec3 center) {
    int viewport[4];
    ...
    gluProject(center.x, center.y, center.z, mvmatrix, projmatrix, viewport, &winx, &winy, &winz);
