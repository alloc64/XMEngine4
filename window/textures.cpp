...
int Texture::LoadCubemap(char **FilePaths) {
    ...
    for (int i = 0; i < 6; i++) {
        switch (GetTextureType(FilePaths[i])) {
            case 0: {
                TexData *t = texture.LoadJPG(FilePaths[i]);
                if (!t) {
            ...
            case 2: {
                TexData *t = texture.LoadDDS(FilePaths[i]);
                if (!t) {
                    printf("error: %s not found\n", FilePaths[i]);
                    continue;
                }
...
int Texture::LoadXMCubemap(char *filename) {
    TexData *t = new TexData();
    if (!t) return -1;

    FILE *pFile = fopen(filename, "rb");
    if (!pFile) {
        printf("error: can't load cubemap %s!\n", filename);
        return -1;
    }
    ...
    fread(header, sizeof(char), 74, pFile);
    fread(signature, sizeof(char), 7, pFile);
    fread(&imageSize, sizeof(int), 1, pFile);

    int oneImageSize = imageSize / 6;
    t->texels = new unsigned char[oneImageSize];
    ...
    for (int i = 0; i < 6; i++) {
        fread(t->texels, oneImageSize, 1, pFile);
        width = height = (int) sqrt(oneImageSize / 3);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + i, 0, GL_RGB, width, height, 1, GL_RGB, GL_UNSIGNED_BYTE,
                     t->texels);
    }
    glDisable(GL_TEXTURE_CUBE_MAP_ARB);

    if (t->texels) delete[] t->texels;
    ...
    printf("ok: cubemap %s loaded sucessfully!\n", filename);
    return cubeMap;
}
...
bool Texture::LoadExtensions() {
    if (!CheckExtension("ARB_texture_compression")) return false;
...
void Texture::SaveTGAScreenShot() {
    int w = window.w;
    int h = window.h;
    SYSTEMTIME st;
    GetSystemTime(&st);
    ...
    sprintf(filename, "Fram3Soft-Alfa-Screen-%d-%d-%d-%d-%d-%d.tga", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute,
            st.wSecond);
    printf("%s\n", filename);
